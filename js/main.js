...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (loader) {
    const minLoadTime = new Promise(r => setTimeout(r, 1800));
  ...
  // Birthday Countdown (March = month 2, day 12)
  new BirthdayCountdown(2, 12);
  ...
  new BirthdayCake(() => {
    // All candles blown! Show gift button
    if (openGiftBtn) {
  ...
  funfactCards.forEach(card => {
    card.addEventListener('click', () => {
      funfactCards.forEach(c => { if (c !== card) c.classList.remove('flipped'); });
